...
/**
 * Listens for the errors a driver client emits in the background, unless it
 * already does (e.g., because the same database is registered under several
 * connection names). A node-postgres pool (and the Neon WebSocket pool built
 * on it) emits `error` when the server drops an idle connection: a restart,
 * a failover, a network partition. The pool has already discarded that
 * connection; with no listener, Node raises the event as an uncaught
 * exception and the process exits. Other clients either handle it themselves
 * (mysql2 listens on each pooled connection, postgres.js reconnects) or are
 * not event emitters (better-sqlite3, libSQL, PGlite, the HTTP drivers), so
 * the listener is attached wherever `on()` exists and is inert elsewhere.
 */
